...
public class FileDeleteHarness extends BaseHarness implements RunHarness {
	...
	public boolean deleteFileContent(String fName) {
		...
		try {
			...
			this.getErrors().add(new HarnessError(this, "File Not Found Exception", "Couldn't find file to delete: " + fName));
			result = false;
		} catch (IOException e) {
	...
	public boolean deleteFolder(File dir) {
		if (dir.isDirectory()) {
	        ...
	        for (int i=0; i<content.length; i++) {
	            ...
	            if (!success) {
	            	this.getErrors().add(new HarnessError(this, "Folder Deletion Exception", "Couldn't delete content of file: " + content[i]));
	                return false;
	            }
	        }
	    }
